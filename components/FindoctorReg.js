...
import ConfirmVisitReg from './ConfirmVisitReg.js'
...
export default class FindoctorReg extends Component {
...
onURGENT(name,clinic,time,duration)
{
this.props.navigation.navigate('ConfirmVisitReg',{name:name,clinic:clinic,time:time,duration:duration});
}
  ...
  oniconbl()
  {
    this.props.navigation.navigate('RegularScreen');//n
        ...
        <View style={{flexDirection:'column',alignItems:'center',flexWrap:'wrap',alignSelf:'left'}}>
         <Text> {''} </Text>
          ...
          <Text> {'' }</Text>
            ...
            <View style={{flexDirection:'column',flexWrap:'wrap',alignItems:'right',aignSelf:'right'}}>
             <Image source={require('./star64.png')} style={{width:60,height:60,}} />
               <Text> {''} </Text>
          <Text> {'' }</Text>					
					  <Text> {''} </Text>
          </View>
